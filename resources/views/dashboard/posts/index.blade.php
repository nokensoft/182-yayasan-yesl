    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-3 mb-6">
        <form method="GET" class="flex flex-wrap items-center gap-2 w-full lg:max-w-3xl">
            ...
            @if (request()->hasAny(['q', 'category', 'status']))
                <a href="{{ route('dashboard.posts.index') }}" class="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm text-slate-500 hover:text-primary transition" title="Reset filter"><i class="fa-solid fa-xmark"></i> Reset</a>
            @endif
        </form>
        <div class="flex items-center gap-2 shrink-0">
            <a href="{{ route('dashboard.posts.trash') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-300 text-sm font-semibold hover:bg-slate-200 dark:hover:bg-white/10 transition"><i class="fa-solid fa-trash-can"></i> Sampah ({{ $trashedCount }})</a>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-white/10">
                    ...
                    @empty
                        <tr><td colspan="5" class="px-5 py-12 text-center text-slate-400">
                            @if (request()->hasAny(['q', 'category', 'status']))
                                Tidak ada artikel yang cocok dengan filter. <a href="{{ route('dashboard.posts.index') }}" class="text-primary font-semibold">Atur ulang filter</a>.
                            @else
                                Belum ada artikel. <a href="{{ route('dashboard.posts.create') }}" class="text-primary font-semibold">Tulis sekarang</a>.
                            @endif
                        </td></tr>
                    @endforelse
                </tbody>
